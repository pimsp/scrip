...
Mention Flyn, Coleman, Balakrishnan et al. and their way of doing quadratic quickly
...
Assume, by looking at a neighborhood of $x$, that $X = \Spec A$ is affine and that $p,t_1,\dots,t_d$ generate $O_{X,x}$ with $t_1,\dots,t_d$ regular on $X$. By shrinking $X$ even more, we may assume that $t = (t_1,\dots,t_d): X \to \A^d_{\Z_p}$ is smooth and hence \'etale. Let $\Tilde{X}_x$ be the blowup of $X$ at $x$. Now consider the blowup $\Tilde{X}_x \to X$ of $X$ at $x$, and let $\Tilde{X}_x^p$ be the part where $p$ generates the maximal ideal. As $t$ is \'etale, the ideal of $X$ defining $x$ is the pullback along $t$ of the ideal of $\A^d_{\Z_p}$ defining the origin $a$ over $\F_p$. That means that the blowup $\Tilde{X}_x \to X$ is the pullback of the blowup $\Tilde{\A}^d_{\Z_p,a} \to \A^d$. Then the part $\Tilde{X}_x^p$ is the pullback of the corresponding part of $\Tilde{\A}^d_{\Z_p,a}$, i.e. $\Spec \Z_p[x_1/p,...,x_d/p]$ with the morphism $\Z_p[x_1,...,x_d] \to \Z_p[x_1/p,...,x_d/p]$ being the inclusion. That implies that $\Tilde{X}_x^p$ is $\Spec A[]$
...
\label{alg:abeljacobi}
 ...
 \eIf{$(f^r)'(0) \not= 0$}{
   $b_{g+4} \leftarrow \frac{X(X^{g+1} + Y)}{Z^{g+2}}$\;
   }{
   $b_{g+4} \leftarrow \frac{X(X^{g+1} + Y + XZ^{g})}{Z^{g+2}}$\;
  }
 $b_{g+4} \leftarrow xF$\;
 $W \leftarrow$ a $(n_Z + 1)\times (g+4)$ matrix with rows being the evaluations of $B\sqcup{b_{g + 4}}$ on a point in $Z'$.\;
 $V \leftarrow \ker(\im W \subset R^{n_Z + 1} \to R)$, the projection on the last coordinate.\;
...
Now, if $g^r(0)$ is non-zero, then $w- \pm 1$ is also a uniformiser at $\infty_\pm$ and non-zero at $\infty_\mp$, so \[(w+1)/v^{g+2} = \frac{X(X^{g+1} - Y)}{Z^{g+2}}\] has poles exactly $(g+2)(\infty_+ + \infty_-) - \infty_-$ as we wanted to show.
And if $g^r(0)$ is zero, then $v_{\infty_\pm}(w-\pm1)$ is at least $2$ so $w-\pm 1 + v$ is a uniformiser at $\infty_\pm$ and non-zero at $\infty_\mp$, so \[(w+1+v)/v^{g+2} = \frac{X(X^{g+1} + Y + XZ^g)}{Z^{g+2}}\] again has the right poles.
\end{proof}


\begin{proof}[Proof of Proposition \ref{prop:abeljacobia}]
First note that by Riemann-Roch, the dimension of $\Lcal(D_0)$ is $g + 3$, and we also have by the proof of the previous lemma that $1,x,\dots,x^{g+1},y$ all lie in $\Lcal(D_0)$ and hence form a basis, so we can indeed find $B$ as in line~\ref{alg:abeljacobi:basisLD0}. Note that by Lemma~\ref{lem:poles2d0i+} the element $b_{g+4}$ lies in $\Lcal(D_0 + \infty_+)$ but not in $\Lcal(D_0)$, so as $\deg D_0 \geq 2g-1$, we have by a dimensional argument that $b_1,\dots,b_{g+4}$ is a basis for $\Lcal(D_0 + \infty_+)$.

Evaluating $\Lcal(D_0 + \infty_+)$ on $P$ gives a linear map $\Lcal(D_0 + \infty_+) \to R$, and the kernel is exactly $\Lcal(D_0 + \infty_+ - P)$. Finally, note that the poles of $F$ are, again by the previous lemma, $g(\infty_+ + \infty_-) + \infty_-$. 
\end{proof}
