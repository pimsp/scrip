...
\subsection{Complications and improvements}
$\overline{J(\Z)_0}$ open in $J(\Z_p)_0$.

Calculations in $J(\Z)$.

According to McCallum\&Poonen, it can be that $J(\Z)_0$ and $C(\Z)_P$ are tangent as $p$-adic manifolds and then finite-precision calculations will never be enough. I don't see what's happening there. Maybe give an example? 

It is possible that $C(\Z_p)_P \cap \overline{J(\Z)_t}$ is bigger than $C(\Z)_P$; give an example. Refer to \citep{Balakrishnan19}.

Explain Mordell-Weil sieve
