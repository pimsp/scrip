...
Finally, let $m \in \Z_{>0}$ with $p > m+1$. As $c_n$ has $p$-adic valuation $n-1$ for $n < p$, and $p$-adic valuation at least $p-2 \geq m$ for $n \geq p$, indeed $\log_p$ has degree at most $m$ modulo $p^m$. Then the statement that $\exp_p$ has degree at most $m$ modulo $p^m$ is easily proven by the formula in $\Z/p^m\Z[x,\varepsilon]$ that $\log_p(g(x) -\varepsilon)+\varepsilon$
\end{proof}
