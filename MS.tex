...
Now that we can do computations with elements in the Jacobian over $\Z/p^2\Z$, it only remains to construct elements in the Jacobian. Explicitly, we want to go from a degree zero divisor to an element in Mascots representation. In our case, we will only need this in cases where divisors are sums of points, and since we can add elements in the Jacobian, for this it is enough to compute the Abel-Jacobi embedding
\begin{align*}
j_{\infty_+}: C &\to J \\
              P &\mapsto P-\infty_+.
\end{align*}
If the degree zero divisor is not a sum of points, one can pass to an extension, knowing that the result at the end will be defined over the ring we started with.
