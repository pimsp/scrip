...
\label{section:intro}
In mathematics a fundamental problem is solving polynomial equations over the rationals, dating back to Diophantus. An important special case in algebraic geometry is that of curves. The behavior of the rational points of curves depends enormously on the \textit{genus} of the curve, a numerical invariant of a curve. For $g = 0$, there are either no or infinitely many solutions, and their behavior is well understood.
...
Say how much is borrowed from the article by Bas and Guido, but with a focus on the linear part, and more explanations. It is meant as a full guide to linear Chabauty, including both theoretical and practical components, focussing partly on explicit computations.
...
We first calculate $\Lcal(2D_0 - D_{\text{aff}})$. Note that \todo{Compute this, currently talking with Mascot about this.}
...
Tell about how $J(\Z_p)_x$ is now parametrised as the product of $C(\Z_p)_{P_i}$, \todo{what this means for mod $p^2$}
